...
import { deleteRowAtReport, updateReport } from "../../../redux/reportTable";
...
function RowData({
  ...
  const deleteRow = () => {
    dispatch(deleteRowAtReport({ route: reportItems.CMP.DELETE_ROW, rID: row.id }));
  };

  const updateReportOnServer = () => {
    dispatch(
      updateReport({
        route: reportItems.CMP.UPDATE_ROW,
        rID: rowData.id,
        body: {
